export const en = {
  ...
  procedurePlanner: {
    ...
    bbcaDisclaimer: "Exploratory research only — BBCA is a preclinical plant-derived agent, outside the L1 thesis scope and outside the zero-contrast pathway. Reported relaxivity is ~3000× lower than gadolinium, so it is not an angiographic contrast agent today. Shown for blue-sky research planning, never for clinical use.",
  ...
  digitalTwin: {
    ...
    segmentDetail: {
      contrastFree: {
        ...
        recBBCA: "BBCA bio-contrast — exploratory research only, not an angiographic agent",
  ...
  fusionViewer: {
    ...
    modalities: {
      ...
      bioMra: {
        ...
        agentDesc: "BBCA (Betalain-Based Contrast Agent) — beetroot-derived, GRAS-classified. Exploratory only: reported relaxivity is ~3000× lower than gadolinium, so it is not an angiographic contrast agent today.",
  ...
  home: {
    ...
    validation: {
      ...
      cards: [
        { title: "Primary endpoint", desc: "Decisional concordance (quadratically weighted κ, 3 therapeutic classes) between the post-mapping decision and the reference multidisciplinary decision; target κ ≥ 0.60 (95 % CI lower bound ≥ 0.45). Anatomical segmental concordance is a territory-stratified confirmatory secondary." },
        { title: "Comparators", desc: "Single dominant primary reference (CTA), pre-specified per patient; MRA if iodine contraindicated; DSA triggered on discordance or interventional indication. Standard Doppler ultrasound as comparator for incremental value." },
        ...
        { title: "Go / No-Go rules", desc: "Pre-specified a priori on the decisional primary: Go (κ ≥ 0.60), Reassessment (0.45–0.60), No-Go (< 0.45). Interim futility analysis at n=100." },
      ],
  ...
  pages: {
    ...
    protocol: {
      ...
      comparators: {
        title: "Comparators",
        subtitle: "Single dominant primary reference (CTA) pre-specified per patient before VASCU-LINK reading; reference-by-reference sensitivity analysis. Verification rule (Begg-Greenes) documented.",
        items: [
          { title: "CT angiography (CTA) — primary reference", desc: "Dominant reference for aorto-iliac and femoro-popliteal segments; reflects real clinical practice." },
          { title: "MR angiography (MRA)", desc: "Alternative reference when iodinated contrast is contraindicated." },
          { title: "Digital subtraction angiography (DSA)", desc: "Triggered only on discordance or interventional indication. Below the knee, DSA is an imperfect standard (may miss patent distal vessels) — below-knee analysis framed as added information value, not concordance against an imperfect reference." },
          { title: "Duplex ultrasound (incremental-value comparator)", desc: "Standard-of-care first line; comparator to quantify the incremental decisional value of the non-contrast map." },
        ],
      },
      endpoints: {
        title: "Endpoints",
        subtitle: "Single primary endpoint (decisional), hierarchical secondaries, uncorrected exploratory.",
        primaryLabel: "Primary endpoint",
        primary: "Decisional concordance (quadratically weighted Cohen's κ, 3 therapeutic classes) between the post-mapping decision and the reference multidisciplinary decision based on standard imaging.",
        primaryTarget: "Target: κ ≥ 0.60 (95 % CI lower bound ≥ 0.45).",
        rows: [
          { metric: "Incremental value vs Duplex/ABI alone (net reclassification)", target: "Descriptive 95 % CI" },
          { metric: "Anatomical concordance — aorto-iliac (weighted κ)", target: "κ ≥ 0.75" },
          { metric: "Anatomical concordance — femoro-popliteal (weighted κ)", target: "κ ≥ 0.65" },
          { metric: "Anatomical concordance — below-the-knee (weighted κ)", target: "Reported, descriptive" },
          { metric: "Segmental sensitivity / specificity (stenosis ≥ 50 %, aorto-femoral)", target: "Se ≥ 85 % · Sp ≥ 80 %" },
          { metric: "Inter-reader reproducibility, central blinded reading (n=30)", target: "κ ≥ 0.70" },
          { metric: "Iodinated contrast & radiation avoidance vs standard pathway", target: "Descriptive 95 % CI" },
          { metric: "Acquisition time · patient tolerance (VAS 0–10)", target: "≤ 25 min · median ≤ 2" },
          { metric: "Exploratory — patent below-knee segments unseen on reference", target: "Descriptive, uncorrected" },
        ],
      },
      stats: {
        ...
        sample: "n ≈ 320 patients to enrol (≈ 250 analysable); two-sided α = 0.05, power = 0.90. Sample size calibrated on the decisional primary endpoint (expected κ = 0.70, minimum acceptable κ = 0.45). Donner & Eliasziw method with intra-patient cluster adjustment (GEE).",
      ...
      qa: {
        ...
        items: [
          ...
          },
          {
            ...
            a: "Primary endpoint: decisional concordance (quadratically weighted κ, 3 therapeutic classes) between the post-mapping decision and the reference multidisciplinary decision, target κ ≥ 0.60 (lower bound 95% CI ≥ 0.45). Anatomical segmental concordance is a territory-stratified confirmatory secondary. n ≈ 320 patients to enrol (≈ 250 evaluable); α = 0.05 two-sided, power = 0.90, sample size calibrated on the decisional primary, Donner & Eliasziw method with intra-patient cluster adjustment (GEE). Missing data handled by multiple imputation by chained equations (MICE, m = 20), with complete-case and tipping-point sensitivity analyses. Futility analysis planned at n=100 (O'Brien-Fleming, no α spending on efficacy)."
          },
          {
      ...
      annexes: {
        ...
        referencesLibrary: {
          ...
          claims: [
            ...
            {
              claim: "Primary endpoint: decisional concordance (quadratically weighted κ, 3 therapeutic classes) vs reference multidisciplinary decision, target κ ≥ 0.60; anatomical concordance confirmatory secondary.",
              refs: [
      ...
      sections: {
        ...
        clinical: {
          ...
          items: [
            ...
            "Clinical questionnaires (WIQ, VascuQol-6, 6-MWT) administered in the patient's language using linguistically validated FR/DE/EN versions; version recorded per patient.",
  ...
  vascscreen: {
    ...
    adr: {
      ...
      items: {
        ...
        "ADR-004": { title: "PROMs in validated FR/DE/EN versions (WIQ · VascuQol-6 · 6-MWT)", rationale: "Linguistically validated instruments matched to the patient's language; version logged per patient for psychometric integrity." },
  ...
  l1: {
    ...
    c4i: {
      ...
      description: "Clinical–imaging concordance (C4-i v11.1, recalibrated on external validation cohort) and patient-reported outcome measures (WIQ · VascuQol-6 · 6-MWT) administered in each patient's language using linguistically validated FR / DE / EN versions; the version used is recorded per patient.",
