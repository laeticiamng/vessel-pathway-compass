export const fr = {
  common: {
    appName: "AquaMR Flow",
    loading: "Chargement...",
    save: "Enregistrer",
    cancel: "Annuler",
    delete: "Supprimer",
    edit: "Modifier",
    create: "Créer",
    add: "Ajouter",
    search: "Rechercher",
    filter: "Filtrer",
    export: "Exporter",
    configure: "Configurer",
    back: "Retour",
    next: "Suivant",
    submit: "Soumettre",
    close: "Fermer",
    confirm: "Confirmer",
    viewAll: "Tout voir",
    comingSoon: "Bientôt disponible",
    betaPreview: "Aperçu Beta",
    notAMedicalDevice: "Ce n'est pas un dispositif médical",
    version: "v1.0 Beta · Prêt pour la conformité",
    signIn: "Se connecter",
    signUp: "S'inscrire",
    signOut: "Se déconnecter",
    getStarted: "Commencer",
    learnMore: "En savoir plus",
    analytics: "Analytiques",
    upload: "Téléverser",
    start: "Démarrer",
    continue: "Continuer",
  },

  medicalCategories: {
    pad: "AOMI",
    aortic: "Aortique",
    venous: "Veineux",
    carotid: "Carotidien",
    "dvt/pe": "TVP / EP",
  },

  sidebar: {
    clinical: "Clinique",
    dashboard: "Tableau de bord",
    aiAssistant: "Assistant IA",
    patients: "Patients",
    digitalTwin: "AquaMR Digital Twin",
    registry: "Registre",
    education: "Formation",
    simulationLab: "Labo de simulation",
    expertNetwork: "Réseau d'experts",
    researchHub: "Pôle recherche",
    analytics: "Analytique",
    betaPreview: "Aperçu Beta",
    innovationLab: "Labo d'innovation",
    administration: "Administration",
    compliance: "Conformité",
    team: "Équipe",
    settings: "Paramètres",
    moreTools: "Plus d'outils",
    patientOutcomes: "Résultats patients",
    clinicalPerformance: "Performance clinique",
    riskCalculator: "Calculateur de risque",
  },

  command: {
    placeholder: "Rechercher modules, patients ou actions...",
    noResults: "Aucun résultat trouvé.",
    navigation: "Navigation",
    dashboard: "Tableau de bord",
    aiAssistant: "Assistant clinique IA",
    patients: "Dossiers patients",
    digitalTwin: "AquaMR Digital Twin",
    registry: "AquaMR Registry",
    education: "AquaMR Academy",
    simulation: "Labo de simulation",
    network: "Réseau d'experts",
    research: "Pôle recherche",
    compliance: "Centre de conformité",
    settings: "Paramètres",
  },

  topBar: {
    searchPlaceholder: "Rechercher ou commande...",
    toggleTheme: "Changer le thème",
  },

  landing: {
    nav: {
      pricing: "Tarifs",
      signIn: "Se connecter",
      explore: "Fonctionnalités",
    },
    hero: {
      badge: "La plateforme tout-en-un pour la médecine vasculaire — bêta gratuite",
      title: "AquaMR Flow",
      headlinePre: "Plateforme clinique IA pour la",
      headlineHighlight: "Médecine Vasculaire",
      subtitle: "Gérez vos patients, générez des rapports IA, suivez les résultats et collaborez avec des experts — une plateforme sécurisée pour la médecine vasculaire.",
      cta: "Essayer gratuitement — sans carte bancaire",
      secondary: "Voir les tarifs",
      socialProof: "Bêta gratuite — ouverte aux spécialistes vasculaires du monde entier",
      dashboardCaption: "Le tableau de bord clinique : vue d'ensemble des patients, distribution des risques et accès rapide à tous les modules.",
    },
    howItWorks: {
      title: "Comment ça marche",
      subtitle: "Démarrez en quelques minutes — aucune installation, aucune configuration complexe.",
      steps: [
        { title: "Créez votre compte gratuit", desc: "Inscrivez-vous en 30 secondes avec votre e-mail ou Google. Sans carte bancaire." },
        { title: "Ajoutez votre premier patient", desc: "Créez un dossier patient pseudonymisé et commencez à documenter avec les rapports assistés par IA." },
        { title: "Explorez tous les modules", desc: "Simulations, formation, registre des résultats et consultations d'experts — tout au même endroit." },
      ],
    },
    modules: {
      title: "Tout ce dont vous avez besoin",
      subtitle: "Six modules essentiels conçus pour votre workflow clinique quotidien.",
      moreTitle: "Et bien plus encore",
      moreSubtitle: "Des outils supplémentaires pour aller plus loin en résultats, performance et interopérabilité.",
      ai: {
        title: "Assistant clinique IA",
        desc: "Générez des comptes-rendus cliniques structurés, diagnostics différentiels et parcours de soins — toujours vérifiés et confirmés par vous.",
      },
      twin: {
        title: "AquaMR Digital Twin",
        desc: "Visualisez l'anatomie vasculaire de chaque patient au fil du temps avec des cartes interactives, le suivi des lésions et la planification de scénarios.",
      },
      registry: {
        title: "AquaMR Registry",
        desc: "Enregistrez et comparez les résultats patients pour l'AOMI, l'aorte, la carotide, le veineux et le thromboembolique — benchmarking anonymisé inclus.",
      },
      education: {
        title: "Certification & FMC",
        desc: "Progressez dans des parcours de compétences avec cours, quiz, carnets supervisés et badges numériques.",
      },
      simulation: {
        title: "Labo de simulation clinique",
        desc: "Entraînez votre prise de décision avec des cas réalistes à embranchements et recevez un feedback IA sur vos performances.",
      },
      network: {
        title: "Réseau d'experts",
        desc: "Discutez de cas anonymisés avec vos pairs, demandez des avis d'experts et trouvez des mentors entre spécialités.",
      },
      outcomes: {
        title: "Résultats patients (PROMs)",
        desc: "Collectez des questionnaires vasculaires validés (VascuQoL-6, CIVIQ-14) sous forme numérique hebdomadaire avec visualisation des tendances.",
      },
      performance: {
        title: "Performance clinique",
        desc: "KPIs en un coup d'œil : taux de complications à 30 jours, délai moyen avant intervention, taux de resténose — comparés aux benchmarks européens.",
      },
      riskCalc: {
        title: "Calculateur de risque",
        desc: "Outils interactifs SCORE2, Wells TVP et interprétation IPS avec résultats instantanés et recommandations thérapeutiques.",
      },
      researchExport: {
        title: "Export recherche",
        desc: "Générez des résumés statistiques agrégés et anonymisés formatés pour soumission au registre — aucune donnée patient individuelle exposée.",
      },
      fhir: {
        title: "Interopérabilité FHIR",
        desc: "Chaque dossier patient affiche un badge de compatibilité FHIR indiquant le statut d'interopérabilité avec les standards HL7.",
      },
    },
    trust: {
      title: "Sécurisé et prêt pour la conformité",
      subtitle: "Conçu pour les environnements cliniques avec des exigences strictes en matière de protection des données.",
      signals: [
        "Traçabilité complète de chaque action",
        "Données patients pseudonymisées — pas de ré-identification dans le benchmarking",
        "Les résultats IA nécessitent toujours la confirmation du clinicien",
        "Espaces institutionnels séparés avec isolation stricte des données",
        "Références basées sur les preuves — jamais de recommandations fabriquées",
        "Accès par rôle : médecins, internes, administrateurs",
      ],
    },
    socialProof: { title: "Conçu pour les spécialistes vasculaires — par des spécialistes vasculaires" },
    testimonials: {
      title: "Ce qu'en disent les premiers utilisateurs",
      disclaimer: "Retours issus de notre programme bêta. Identités confidentielles à la demande des participants.",
      items: [
        { quote: "Les rapports IA structurés me font gagner au moins 30 minutes par cas. Avant, je dictais tout — maintenant je relis et je valide. C'est entré dans ma routine quotidienne.", author: "Chirurgien vasculaire", role: "Hôpital universitaire, UE" },
        { quote: "J'utilise le jumeau numérique pendant les visites pour montrer aux internes exactement où sont les lésions et comment elles ont évolué. Ça a remplacé nos schémas au tableau.", author: "Radiologue interventionnel", role: "Centre vasculaire privé, CH" },
        { quote: "En tant qu'interne de 3e année, le labo de simulation et le carnet de stage m'ont aidé à suivre ma progression et préparer mon examen. Rien d'autre ne proposait ça.", author: "Interne en chirurgie vasculaire", role: "CHU, USA" },
      ],
    },
    faq: {
      title: "Questions fréquentes",
      subtitle: "Tout ce que vous devez savoir sur AquaMR Flow.",
      items: [
        { q: "Qu'est-ce que AquaMR Flow ?", a: "AquaMR Flow est une plateforme clinique tout-en-un pour la médecine vasculaire. Elle combine gestion des patients, rapports assistés par IA, suivi des résultats, formation, simulation et réseau d'experts dans un seul outil." },
        { q: "À qui s'adresse AquaMR Flow ?", a: "Aux chirurgiens vasculaires, angiologues, radiologues interventionnels, internes et services hospitaliers spécialisés en médecine vasculaire." },
        { q: "Combien ça coûte ?", a: "Pendant la bêta gratuite, toutes les fonctionnalités sont accessibles sans frais. Après le lancement : un plan Individuel gratuit, un plan Professionnel à 99 $/mois et des tarifs Institution sur mesure." },
        { q: "Est-ce un dispositif médical ?", a: "Non. AquaMR Flow est un outil de documentation et de workflow clinique. Ce n'est pas un dispositif médical certifié. Tous les résultats IA nécessitent votre vérification." },
        { q: "Mes données sont-elles sécurisées ?", a: "Oui. Toutes les données sont chiffrées, pseudonymisées et stockées avec des journaux d'audit complets. La plateforme est conçue en conformité RGPD, HIPAA et nLPD suisse." },
        { q: "Mon équipe peut-elle l'utiliser ?", a: "Absolument. Le plan Institution prend en charge les équipes avec registres partagés, analyses et permissions par rôle." },
      ],
    },
    about: {
      title: "Qui sommes-nous",
      description: "AquaMR Flow est développé par EMOTIONSCARE, une entreprise de santé numérique basée à Amiens, en France. Fondée par des cliniciens et des ingénieurs, notre mission est d'offrir aux professionnels vasculaires une plateforme dédiée et moderne — conçue autour de leurs vrais workflows, pas de modèles génériques.",
      foundedLine: "Fondée à Amiens, France — EMOTIONSCARE SASU",
      highlights: {
        clinical: {
          title: "Conçu par des cliniciens",
          desc: "Chaque fonctionnalité est pensée pour les workflows cliniques réels, validée par des spécialistes vasculaires en exercice.",
        },
        privacy: {
          title: "Confidentialité d'abord",
          desc: "Données pseudonymisées, traçabilité complète et conformité RGPD, HIPAA et nLPD suisse dès le premier jour.",
        },
        community: {
          title: "Communauté en croissance",
          desc: "Rejoignez un réseau de professionnels vasculaires partageant connaissances, benchmarks et bonnes pratiques entre institutions.",
        },
      },
    },
    cta: {
      title: "Prêt à simplifier votre pratique vasculaire ?",
      subtitle: "Rejoignez les spécialistes vasculaires qui explorent déjà la plateforme — gratuit pendant la bêta.",
      button: "Créer un compte gratuit",
    },
    footer: {
      tagline: "La plateforme clinique conçue pour les professionnels de la médecine vasculaire.",
      notMedicalDevice: "Aide à la décision clinique — ce n'est pas un dispositif médical.",
      product: "Produit",
      legal: "Juridique",
      contact: "Contact & Support",
      rights: "Tous droits réservés.",
      skipToContent: "Aller au contenu",
    },
  },

  pricing: {
    title: "Plans et tarifs",
    betaBadge: "🎉 Bêta en cours — accès gratuit à toutes les fonctionnalités",
    betaBannerTitle: "🚀 Toutes les fonctionnalités sont gratuites pendant la bêta",
    betaBannerDesc: "Sans carte bancaire. Sans limite de temps. Explorez tous les modules gratuitement — les tarifs s'appliqueront après le lancement officiel.",
    subtitle: "Du médecin individuel aux grandes institutions — trouvez le plan adapté à votre pratique vasculaire.",
    betaNote: "Pendant la bêta, les fonctionnalités payantes sont accessibles sans facturation. Les tarifs affichés s'appliqueront au lancement officiel.",
    mostPopular: "Le plus populaire",
    currentPlan: "Votre plan",
    managePlan: "Gérer l'abonnement",
    plans: {
      individual: {
        name: "Individuel",
        price: "Gratuit",
        period: "pour toujours",
        desc: "Pour les médecins individuels découvrant la plateforme",
        features: [
          "Assistant clinique IA (limité)",
          "5 dossiers patients",
          "Accès au AquaMR Academy",
          "Accès au forum communautaire",
        ],
        cta: "Commencer gratuitement",
      },
      professional: {
        name: "Professionnel",
        price: "99 €",
        period: "/mois",
        desc: "Pour les médecins vasculaires en exercice",
        features: [
          "Assistant IA illimité",
          "Dossiers patients illimités",
          "AquaMR Digital Twin + Chronologie",
          "AquaMR Registry",
          "Formation complète + certification",
          "Accès au labo de simulation",
          "Consultations d'experts (5/mois)",
        ],
        cta: "Accès bêta gratuit",
      },
      institution: {
        name: "Établissement",
        price: "Sur devis",
        period: "par utilisateur",
        desc: "Pour les hôpitaux et centres vasculaires",
        features: [
          "Tout le plan Professionnel",
          "Espace multi-établissements",
          "Tableaux de bord agrégés",
          "Analytiques de benchmarking",
          "Pôle recherche + constructeur d'études",
          "Centre de conformité",
          "Intégration SSO",
          "Support dédié",
        ],
        cta: "Contacter les ventes",
      },
    },
  },

  auth: {
    createAccount: "Créer un compte",
    welcomeBack: "Bon retour",
    signUpDesc: "Rejoignez la plateforme mondiale de médecine vasculaire",
    signInDesc: "Connectez-vous à votre compte AquaMR Flow",
    email: "E-mail",
    emailPlaceholder: "vous@hopital.fr",
    password: "Mot de passe",
    passwordPlaceholder: "••••••••",
    createBtn: "Créer un compte",
    signInBtn: "Se connecter",
    switchToSignIn: "Déjà un compte ? Se connecter",
    switchToSignUp: "Pas encore de compte ? S'inscrire",
    checkEmail: "Vérifiez votre e-mail",
    checkEmailDesc: "Nous vous avons envoyé un lien de confirmation pour vérifier votre compte.",
    error: "Erreur",
    authRequired: "Authentification requise",
    authRequiredDesc: "Veuillez vous connecter pour utiliser l'assistant IA.",
    forgotPassword: "Mot de passe oublié ?",
    forgotPasswordDesc: "Entrez votre e-mail et nous vous enverrons un lien de réinitialisation.",
    sendResetLink: "Envoyer le lien",
    resetEmailSent: "Nous vous avons envoyé un lien de réinitialisation. Vérifiez votre boîte de réception.",
    backToSignIn: "Retour à la connexion",
    newPassword: "Nouveau mot de passe",
    newPasswordDesc: "Entrez votre nouveau mot de passe ci-dessous.",
    confirmPassword: "Confirmer le mot de passe",
    passwordMismatch: "Les mots de passe ne correspondent pas.",
    updatePassword: "Mettre à jour",
    passwordUpdated: "Mot de passe mis à jour",
    passwordUpdatedDesc: "Votre mot de passe a été mis à jour avec succès.",
    invalidLink: "Lien invalide ou expiré",
    invalidLinkDesc: "Ce lien de réinitialisation n'est plus valide. Veuillez en demander un nouveau.",
    acceptTerms: "J'accepte les",
    acceptTermsHint: "Vos données sont chiffrées et jamais partagées. Vous pouvez supprimer votre compte à tout moment.",
    passwordWeak: "Faible — ajoutez des majuscules, chiffres ou symboles",
    passwordMedium: "Moyen — ajoutez plus de variété",
    passwordStrong: "Mot de passe fort",
    continueWithGoogle: "Continuer avec Google",
    orContinueWith: "ou continuer par email",
    confirmationTitle: "Vérifiez votre boîte de réception",
    confirmationDesc: "Nous avons envoyé un lien de confirmation à",
    confirmationNote: "Cliquez sur le lien dans l'e-mail pour activer votre compte. Si vous ne le voyez pas, vérifiez votre dossier spam.",
    resendEmail: "Renvoyer l'e-mail de confirmation",
    resendSuccess: "E-mail de confirmation renvoyé avec succès.",
    backToLogin: "Retour à la connexion",
  },

  notFound: {
    title: "404",
    message: "Oups ! Page non trouvée",
    backHome: "Retour à l'accueil",
  },

  dashboard: {
    title: "Tableau de bord clinique",
    welcome: "Bon retour. Voici votre aperçu clinique.",
    checklist: {
      title: "Premiers pas",
      subtitle: "Complétez ces étapes pour configurer votre espace",
      createPatient: "Créez votre premier patient",
      generateAI: "Générez un rapport IA",
      exploreEducation: "Explorez le AquaMR Academy",
      runSimulation: "Lancez une simulation clinique",
    },
    quickActions: {
      newCase: "Nouveau dossier patient",
      aiAssistant: "Assistant IA",
      simulation: "Démarrer une simulation",
      education: "Continuer la formation",
    },
    recentActivity: "Activité récente",
    recentActivityDesc: "Vos dernières actions dans tous les modules",
    noActivity: "Aucune activité récente",
    modules: "Modules cliniques",
    stats: {
      activeCases: "Dossiers actifs",
      aiReports: "Rapports IA",
      outcomes: "Résultats",
      registryEntries: "Entrées du registre",
      completed: "Terminées",
      generated: "Générés",
      cmeCredits: "Crédits FMC",
      simulations: "Simulations",
    },
    moduleDesc: {
      ai: "Générez des rapports cliniques assistés par IA avec notes structurées et diagnostics différentiels.",
      twin: "Visualisez les chronologies patient, les cartes vasculaires et lancez des simulations de scénarios.",
      registry: "Contribuez et analysez les données de résultats vasculaires avec benchmarking respectueux de la vie privée.",
      education: "Complétez les parcours de compétences, obtenez des crédits FMC et collectez des badges numériques.",
      simulation: "Pratiquez la prise de décision clinique avec des scénarios interactifs à embranchements.",
      research: "Concevez des études, gérez des cohortes et exportez des jeux de données dé-identifiés.",
    },
    riskDistribution: {
      title: "Aperçu des risques patients",
      desc: "Distribution des risques dans votre cohorte",
      viewAll: "Voir tout",
      noPatients: "Aucun patient enregistré",
      totalPatients: "patients au total",
    },
    activityLabels: {
      ai_report_generated: "Rapport IA généré",
      ai_report_signed_off: "Rapport IA validé",
      patient_created: "Patient créé",
      case_created: "Cas créé",
      measurement_added: "Mesure ajoutée",
      event_added: "Événement ajouté",
      outcome_recorded: "Résultat enregistré",
    },
  },

  aiAssistant: {
    title: "Assistant clinique IA",
    subtitle: "Générez des rapports cliniques structurés avec analyse assistée par IA",
    badge: "Confirmation du clinicien requise",
    disclaimer: {
      title: "Contenu généré par IA — Pas un diagnostic",
      body: "Toutes les sorties nécessitent la revue et la confirmation d'un clinicien. Les champs de citation utilisent des références placeholder. Cet outil ne fournit ni avis médical ni diagnostic.",
    },
    signInPrompt: "Connectez-vous pour générer et sauvegarder des rapports IA avec traçabilité complète.",
    intake: {
      title: "Données cliniques",
      subtitle: "Saisissez les données du patient pour l'analyse IA",
      symptoms: "Symptômes et présentation",
      symptomsPlaceholder: "ex. : Claudication intermittente, douleurs de repos, périmètre de marche 200m...",
      riskFactors: "Facteurs de risque",
      riskFactorsPlaceholder: "ex. : Diabète, hypertension, tabagisme 30 paquets-années...",
      abi: "IPS / ABI",
      abiPlaceholder: "ex. : Droit : 0,65, Gauche : 0,82",
      doppler: "Résumé Doppler",
      dopplerPlaceholder: "ex. : Flux monophasique AFS droite",
      imaging: "Résumé CTA/ARM",
      imagingPlaceholder: "ex. : Occlusion de l'AFS droite, poplitée perméable...",
      labs: "Bilan biologique",
      labsPlaceholder: "ex. : HbA1c 7,8%, LDL 1,6 g/L",
      medications: "Traitements en cours",
      medicationsPlaceholder: "ex. : Aspirine, Statine, Cilostazol",
    },
    generate: "Générer le rapport",
    generating: "Génération en cours...",
    output: {
      title: "Rapport généré par IA",
      subtitle: "Relire, confirmer et exporter",
      report: "Rapport",
      evidence: "Preuves",
      empty: "Saisissez les données cliniques et générez un rapport",
      dataUsed: "Données utilisées",
      dataUsedDesc: "Données cliniques fournies par le patient via le formulaire d'entrée.",
      uncertainty: "Incertitude et limites",
      uncertaintyDesc: "L'analyse IA repose uniquement sur les données structurées saisies. Les recommandations utilisent des citations placeholder.",
      clinicianRequired: "Confirmation du clinicien requise",
      clinicianRequiredDesc: "Cette sortie doit être revue par un clinicien qualifié avant toute action clinique.",
      copyEHR: "Copier vers le DPI",
      copied: "Copié",
      copiedDesc: "Rapport copié en texte brut (format DPI)",
      exportPDF: "Exporter PDF",
      exportPDFSoon: "L'export PDF arrive bientôt. Utilisez « Copier vers DPI » en attendant.",
      confirmSign: "Confirmer et signer",
      signedOff: "Signé",
      signedOffDesc: "Rapport confirmé et signé par le clinicien.",
    },
    history: {
      title: "Historique des rapports",
      subtitle: "Rapports IA précédents avec piste d'audit",
      signed: "Signé",
      pending: "En attente",
      empty: "Aucun rapport encore",
      emptyDesc: "Les rapports générés apparaîtront ici avec la traçabilité complète.",
    },
    errors: {
      rateLimited: "Limite de débit atteinte",
      rateLimitedDesc: "Veuillez réessayer dans un instant.",
      creditsRequired: "Crédits nécessaires",
      creditsRequiredDesc: "Veuillez ajouter des crédits pour continuer.",
      signOffFailed: "Échec de la validation. Veuillez réessayer.",
    },
  },

  patients: {
    title: "Dossiers patients",
    subtitle: "Gérez les dossiers vasculaires pseudonymisés",
    newCase: "Nouveau dossier",
    searchPlaceholder: "Rechercher des dossiers...",
    empty: "Aucun dossier patient",
    columns: {
      caseId: "ID dossier",
      category: "Catégorie",
      status: "Statut",
      abi: "IPS",
      risk: "Risque",
      lastVisit: "Dernière visite",
    },
    form: {
      description: "Créer un nouveau dossier patient pseudonymisé",
      pseudonym: "Pseudonyme",
      ageRange: "Tranche d'âge",
      sex: "Sexe",
      male: "Masculin",
      female: "Féminin",
      category: "Catégorie",
      caseTitle: "Titre du dossier",
      pseudonymPlaceholder: "ex. : PAT-2026-001",
      caseTitlePlaceholder: "ex. : Occlusion AFS droite",
      selectPlaceholder: "Sélectionner",
    },
    filters: {
      allCategories: "Toutes les catégories",
      allStatuses: "Tous les statuts",
      allRisks: "Tous les risques",
      clearAll: "Effacer les filtres",
    },
    risk: {
      low: "Faible",
      moderate: "Modéré",
      high: "Élevé",
      critical: "Critique",
    },
    statuses: {
      active: "Actif",
      completed: "Terminé",
      archived: "Archivé",
    },
  },

  digitalTwin: {
    title: "AquaMR Digital Twin",
    subtitle: "AquaMR Digital Twin interactive avec mesures réelles du patient",
    selectPatient: "Sélectionner un patient...",
    selectPatientPrompt: "Sélectionnez un patient pour visualiser son anatomie vasculaire avec ses mesures réelles",
    selectSegment: "Cliquez sur un segment vasculaire sur la carte pour voir les mesures",
    cases: "cas",
    tabs: {
      timeline: "Chronologie",
      vascularMap: "Carte vasculaire",
      carePlan: "Cas et plan de soins",
    },
    vascularMap: {
      title: "Carte vasculaire",
      clickPrompt: "Cliquez sur un segment pour voir les mesures",
    },
    legend: {
      normal: "Normal",
      warning: "Attention",
      critical: "Critique",
    },
    noMeasurements: "Aucune mesure enregistrée pour ce segment",
    noEvents: "Aucun événement clinique enregistré",
    noCases: "Aucun cas pour ce patient",
    readings: "mesures",
    riskFactors: "Facteurs de risque",
    carePlan: {
      title: "Cas du patient",
    },
  },

  registry: {
    title: "AquaMR Registry",
    subtitle: "Données de résultats vasculaires structurées avec benchmarking respectueux de la vie privée",
    badge: "Données anonymisées uniquement",
    tabs: {
      physician: "Mes résultats",
      institution: "Établissement",
      benchmarking: "Benchmarking",
    },
    stats: {
      casesContributed: "Dossiers contribués",
      avgPatency: "Perméabilité moy.",
      amputationRate: "Taux d'amputation",
      mortality30day: "Mortalité à 30 jours",
      complicationRate: "Taux de complications",
      promsCollected: "PROMs collectés",
    },
    emptyOutcomes: "Aucune donnée de résultats pour le moment. Ajoutez des résultats depuis les dossiers patients pour peupler ce registre.",
    institutionAggregate: "Agrégat de l'établissement",
    networkBenchmarking: "Benchmarking réseau",
    privacyFirst: "Vie privée d'abord",
    table: {
      category: "Catégorie",
      entries: "Entrées",
      amputation: "Amputation",
      restenosis: "Resténose",
      mortality: "Mortalité",
      complications: "Complications",
    },
    institutionPlaceholder: "Tableau de bord agrégé de l'établissement — nécessite le rôle Admin Hospitalier",
    benchmarkPlaceholder: "Comparaison par centiles avec le registre mondial — anonymisé",
  },

  education: {
    title: "AquaMR Academy",
    subtitle: "Parcours de compétences, certification et crédits FMC",
    cmeCredits: "Crédits FMC",
    stats: {
      modulesCompleted: "Quiz réussis",
      badgesEarned: "Badges obtenus",
      tracksInProgress: "Parcours en cours",
    },
    tracks: "Cours",
    quizzesPassed: "quiz réussis",
    digitalBadges: "Badges numériques",
    badges: {
      verified: "Vérifié",
      inProgress: "En cours",
    },
    badgesDesc: "Attestations internes avec métadonnées vérifiables",
    certificate: {
      title: "Cours terminé !",
      desc: "Téléchargez votre certificat de complétion",
      download: "Certificat PDF",
      downloaded: "Certificat téléchargé",
      error: "Échec de la génération du certificat",
    },
    emptyCourses: "Aucun cours. Créez le premier !",
    createCourse: "Nouveau cours",
    createCourseDesc: "Créez un cours de compétences avec modules et quiz",
    courseCreated: "Cours créé avec succès",
    errorCreating: "Erreur lors de la création",
    backToCourses: "Retour aux cours",
    noModules: "Aucun module dans ce cours.",
    fields: {
      title: "Titre",
      description: "Description",
      track: "Parcours",
      difficulty: "Difficulté",
      duration: "Durée (heures)",
    },
    placeholders: {
      title: "ex. : Fondamentaux de l'échographie vasculaire",
      description: "Décrivez le contenu du cours...",
    },
    difficulties: {
      beginner: "Débutant",
      intermediate: "Intermédiaire",
      advanced: "Avancé",
    },
    trackNames: {
      vascularUltrasound: "Échographie vasculaire",
      padLimb: "AOMI / Membre",
      aorta: "Aorte",
      venous: "Veineux",
      thrombosis: "Thrombose",
    },
    quiz: {
      take: "Passer le quiz",
      retry: "Réessayer",
      submit: "Soumettre les réponses",
      passed: "Réussi !",
      failed: "Non réussi.",
      passingScore: "Score de passage",
      backToCourse: "Retour au cours",
      errorSubmitting: "Erreur lors de la soumission",
    },
    modules: {
      add: "Ajouter un module",
      addDesc: "Ajoutez un cours, une vidéo ou un module pratique à ce cours",
      created: "Module créé avec succès",
      deleted: "Module supprimé",
      errorCreating: "Erreur lors de la création du module",
      type: "Type de module",
      content: "Contenu",
      titlePlaceholder: "ex. : Bases de l'échographie Duplex",
      contentPlaceholder: "Rédigez le contenu du cours ou les instructions...",
      types: {
        lesson: "Leçon",
        video: "Vidéo",
        caseStudy: "Étude de cas",
        practical: "Pratique",
      },
    },
    quizBuilder: {
      add: "Ajouter un quiz",
      addDesc: "Créez un quiz avec des questions à choix multiples pour un module",
      created: "Quiz créé avec succès",
      deleted: "Quiz supprimé",
      errorCreating: "Erreur lors de la création du quiz",
      module: "Module",
      selectModule: "Sélectionnez un module",
      titlePlaceholder: "ex. : Évaluation du module 1",
      questions: "Questions",
      question: "Question",
      addQuestion: "Ajouter une question",
      questionPlaceholder: "Saisissez la question...",
      option: "Option",
      selectCorrect: "Cliquez sur le cercle pour indiquer la bonne réponse",
    },
  },

  simulation: {
    title: "Labo de simulation clinique",
    subtitle: "Cas interactifs à embranchements avec feedback IA",
    tabs: {
      cases: "Bibliothèque de cas",
      history: "Mes essais",
      heatmap: "Carte des compétences",
    },
    heatmap: {
      title: "Carte des compétences",
      subtitle: "Performance dans les domaines clés de compétence",
    },
    createCase: "Créer un nouveau cas",
    createDesc: "Définissez un scénario clinique à embranchements avec rubriques de notation",
    created: "Simulation créée avec succès",
    errorCreating: "Erreur lors de la création",
    empty: "Aucune simulation. Créez la première !",
    retry: "Réessayer",
    finish: "Terminer",
    step: "Étape",
    backToList: "Retour aux simulations",
    noSteps: "Cette simulation n'a pas d'étapes configurées.",
    noRuns: "Aucun essai terminé. Lancez une simulation !",
    historyDesc: "Vos essais passés et vos scores",
    runSaved: "Essai enregistré",
    errorSaving: "Erreur lors de l'enregistrement",
    stats: {
      available: "Cas disponibles",
      completed: "Essais terminés",
      avgScore: "Score moyen",
    },
    fields: {
      title: "Titre",
      description: "Description",
      category: "Catégorie",
      difficulty: "Difficulté",
      timeLimit: "Limite de temps (secondes)",
    },
    placeholders: {
      title: "ex. : Ischémie aiguë de membre — Triage d'urgence",
      description: "Décrivez le scénario clinique...",
    },
    difficulty: { beginner: "Débutant", intermediate: "Intermédiaire", advanced: "Avancé", hard: "Difficile", medium: "Moyen", easy: "Facile" },
    categories: {
      pad: "AOMI",
      aortic: "Aortique",
      venous: "Veineux",
      carotid: "Carotidien",
    },
    skills: {
      triageAccuracy: "Précision du triage",
      safetySteps: "Étapes de sécurité",
      documentation: "Documentation",
      communication: "Communication",
    },
    results: {
      title: "Simulation terminée",
      passed: "Bravo ! Vous avez réussi.",
      needsWork: "Continuez à vous entraîner pour améliorer votre score.",
      duration: "Durée",
      answered: "Répondu",
      rubrics: "Scores par rubrique",
    },
  },

  network: {
    title: "Réseau mondial d'experts",
    subtitle: "Discussions de cas dé-identifiés, télé-expertise et mentorat",
    tabs: {
      discussions: "Discussions",
      askExpert: "Demander à un expert",
      mentorship: "Mentorat",
    },
    newDiscussion: "Nouvelle discussion",
    searchDiscussions: "Rechercher des discussions...",
    submitCase: "Soumettre un cas",
    askExpertDesc: "Soumettez un cas dé-identifié pour une revue d'expert structurée avec réponse sous 48h.",
    requestMentorship: "Demander un mentorat",
    rating: "Note",
    cases: "cas",
    errorTitle: "Erreur",
    dialogLabels: {
      title: "Titre",
      topic: "Sujet",
      content: "Contenu",
      selectTopic: "Sélectionner un sujet",
      caseSummary: "Résumé de cas dé-identifié",
    },
    placeholders: {
      postTitle: "ex. : Approche d'occlusion iliaque complexe",
      postContent: "Décrivez le cas ou la question...",
      expertTitle: "ex. : Pathologie aortique inhabituelle",
      expertSummary: "Présentez le scénario clinique dé-identifié...",
    },
    emptyDiscussions: "Aucune discussion pour le moment. Lancez la première !",
    emptyExperts: "Aucune demande d'expert pour le moment",
    mentorshipComingSoon: "Mise en relation de mentorat — bientôt disponible",
    newDiscussionDesc: "Lancez une discussion de cas dé-identifié",
    thread: {
      backToDiscussions: "Retour aux discussions",
      replies: "Réponses",
      noReplies: "Aucune réponse pour le moment. Soyez le premier à répondre !",
      writeReply: "Écrire une réponse",
      replyPlaceholder: "Partagez votre expérience clinique...",
      reply: "Répondre",
      replyPosted: "Réponse publiée avec succès",
      notFound: "Discussion introuvable",
    },
  },

  research: {
    title: "Pôle recherche",
    subtitle: "Constructeur d'études, analytiques et exports de données dé-identifiées",
    newStudy: "Nouvelle étude",
    stats: {
      activeStudies: "Études actives",
      eligiblePatients: "Patients éligibles",
      dataExports: "Exports de données",
    },
    members: "membres",
    eligible: "éligibles",
    pi: "IP",
    statuses: {
      recruiting: "Recrutement",
      active: "Active",
      draft: "Brouillon",
    },
    empty: "Aucune étude pour le moment. Créez votre première étude.",
    studyCreated: "Étude créée avec succès",
    newStudyDesc: "Créer une nouvelle étude de recherche",
    form: {
      title: "Titre de l'étude",
      titlePlaceholder: "ex. Étude multicentrique sur les résultats PAD",
      description: "Description",
      descPlaceholder: "Décrivez les objectifs de l'étude...",
      status: "Statut",
    },
  },

  compliance: {
    title: "Centre de conformité et sécurité",
    subtitle: "Piste d'audit, gestion du consentement et surveillance de la sécurité IA",
    stats: {
      auditEvents: "Événements d'audit",
      consentsActive: "Consentements actifs",
      aiOutputsLogged: "Sorties IA journalisées",
      issuesReported: "Incidents signalés",
    },
    tabs: {
      audit: "Journal d'audit",
      consent: "Gestion du consentement",
      aiSafety: "Sécurité IA",
    },
    table: {
      action: "Action",
      user: "Utilisateur",
      time: "Heure",
      type: "Type",
      status: "Statut",
    },
    consentItems: [
      "Consentement au traitement des données patient",
      "Consentement à la participation à la recherche",
      "Consentement à l'analyse assistée par IA",
    ],
    aiSafetyMetrics: {
      modelVersion: "Version du modèle",
      signOffRate: "Taux de validation",
      issuesReported: "Incidents signalés",
    },
    empty: {
      audit: "Aucun journal d'audit enregistré",
      consent: "Aucun enregistrement de consentement",
      aiOutputs: "Aucune sortie IA enregistrée",
    },
    loadMore: "Charger plus",
    consent: {
      granted: "accordé(s)",
      revoked: "révoqué(s)",
    },
  },

  team: {
    title: "Gestion de l'équipe",
    subtitle: "Gérez les membres et les rôles de l'espace de travail institutionnel",
    inviteMember: "Inviter un membre",
    emptyState: "Rejoignez une institution ou contactez votre administrateur pour accéder à votre équipe. Configurez votre institution dans les Paramètres.",
    columns: {
      name: "Nom",
      email: "E-mail",
      role: "Rôle",
      status: "Statut",
    },
    statuses: {
      active: "Actif",
      invited: "Invité",
    },
    roles: {
      physician: "Médecin",
      hospitalAdmin: "Administrateur hospitalier",
      trainee: "Stagiaire",
      expertReviewer: "Expert évaluateur",
      researchLead: "Responsable de recherche",
    },
  },

  settings: {
    title: "Paramètres",
    subtitle: "Établissement, intégrations et préférences",
    profile: {
      title: "Profil",
      desc: "Vos informations personnelles",
      displayName: "Nom d'affichage",
      displayNamePlaceholder: "Dr. Jean Dupont",
      role: "Rôle",
      rolePlaceholder: "ex. Chirurgien vasculaire",
      institution: "Établissement",
      saved: "Profil enregistré",
    },
    institution: {
      title: "Établissement",
      name: "Nom de l'établissement",
      namePlaceholder: "Hôpital Universitaire",
      country: "Pays",
      countryPlaceholder: "Suisse",
    },
    language: {
      title: "Langue",
    },
    appearance: {
      title: "Apparence",
      darkMode: "Mode sombre",
    },
    security: {
      title: "Sécurité",
      desc: "Votre compte est protégé par des sessions chiffrées, un contrôle d'accès par rôle et une traçabilité complète. Le changement de mot de passe peut être initié depuis la page de connexion.",
    },
    plan: {
      title: "Plan & Facturation",
      desc: "Gérez votre abonnement et vos informations de facturation",
      currentPlan: "Plan actuel",
      free: "Gratuit",
      viewPlans: "Voir les plans",
      renewsOn: "Renouvellement le",
    },
  },

  federated: {
    title: "Apprentissage fédéré",
    subtitle: "Entraînement IA collaboratif respectueux de la vie privée — mises à jour du modèle partagées, données restent locales.",
    optIn: {
      title: "Participation de l'établissement",
      desc: "Activez la participation de votre établissement à l'apprentissage fédéré",
      toggle: "Participer à l'apprentissage fédéré",
      toggleDesc: "L'entraînement se fait localement, seules les mises à jour du modèle sont partagées",
      ethicsApproval: "Approbation éthique",
      ethicsDesc: "Téléversez la documentation d'approbation du comité d'éthique",
    },
    status: {
      title: "Statut du nœud fédéré",
      nodeStatus: "Statut du nœud",
      inactive: "Inactif",
      trainingRounds: "Tours d'entraînement",
      modelVersion: "Version du modèle",
    },
  },

  aiSafetyPage: {
    title: "Couche de sécurité IA",
    subtitle: "MLOps de niveau médical — versionnement des modèles, détection de dérive et suivi des corrections humaines.",
    stats: {
      modelVersion: "Version du modèle",
      outputsLogged: "Sorties journalisées",
      issuesReported: "Incidents signalés",
      signOffRate: "Taux de validation",
    },
    drift: {
      title: "Détection de dérive",
      subtitle: "Surveillez les performances du modèle et les changements de distribution des données",
      placeholder: "Tableau de bord de détection de dérive — bientôt disponible",
    },
  },

  imaging: {
    title: "Pipeline d'imagerie",
    subtitle: "Téléversez des résumés d'imagerie, des mesures structurées et des outils d'annotation. Architecture compatible DICOM.",
    uploadSummary: "Téléverser un résumé",
    uploadDesc: "Téléversez des résumés de rapports d'imagerie",
    measurements: "Mesures",
    measurementsDesc: "Diamètres d'anévrismes, gradation de sténoses",
    annotation: "Outil d'annotation",
    annotationDesc: "Marquer et annoter les résultats d'imagerie",
    dicom: {
      title: "Architecture DICOM",
      subtitle: "Futur : Pipeline d'ingestion DICOM directe — architecture placeholder",
      placeholder: "Intégration DICOM — architecture prête",
    },
  },

  wearables: {
    title: "Objets connectés & Suivi à domicile",
    subtitle: "Concept d'application patient pour tests de marche, journal des symptômes, photos de plaies et observance de la compression.",
    features: {
      walking: { title: "Suivi des tests de marche", desc: "Test de marche de 6 minutes, distance de claudication" },
      symptom: { title: "Journal des symptômes", desc: "Suivi quotidien des symptômes et tendances" },
      wound: { title: "Journal photo des plaies", desc: "Documentation séquentielle des plaies" },
      compression: { title: "Observance de la compression", desc: "Suivi de l'observance de la thérapie compressive" },
    },
    consent: {
      title: "Consentement et partage de données du patient",
      items: [
        "Partager les données du test de marche avec le médecin",
        "Partager les photos de plaies avec l'équipe soignante",
        "Autoriser les données à des fins de recherche",
      ],
    },
  },

  arTraining: {
    title: "Formation immersive (RA)",
    subtitle: "Mode check-list RA et formation échographique par stations.",
    checklist: {
      title: "Mode check-list RA",
      subtitle: "Concept de superposition de check-list procédurale étape par étape",
      steps: {
        s1: "Identification du patient et consentement",
        s2: "Préparation du matériel et sélection de la sonde",
        s3: "Identification des repères anatomiques",
        s4: "Protocole de balayage vasculaire systématique",
        s5: "Acquisition des mesures et documentation",
        s6: "Génération du rapport et validation",
      },
    },
    station: {
      title: "Formation par stations",
      subtitle: "Stations de formation à l'acquisition échographique — concept UI",
      placeholder: "Environnement de formation échographique par stations — bientôt disponible",
    },
  },

  // Patient Detail
  patientDetail: {
    back: "Retour",
    notFound: "Patient non trouvé.",
    age: "Âge",
    cases: "Dossiers",
    timelineEvents: "Événements",
    measurementsCount: "Mesures",
    measurementTrends: "Tendances des mesures",
    riskFactors: {
      title: "Facteurs de risque",
      none: "Aucun facteur de risque enregistré",
      quickAdd: "Ajout rapide :",
      customPlaceholder: "Facteur de risque personnalisé...",
      low: "Risque faible",
      moderate: "Risque modéré",
      high: "Risque élevé",
      critical: "Risque critique",
    },
    tabs: {
      timeline: "Chronologie",
      measurements: "Mesures",
      cases: "Dossiers",
    },
    caseTimeline: "Chronologie du dossier",
    addEvent: "Ajouter un événement",
    bulkDelete: "Supprimer la sélection",
    selectAll: "Tout sélectionner",
    addFirstEvent: "Ajouter le premier événement",
    noEvents: "Aucun événement dans la chronologie",
    measurements: "Mesures",
    addMeasurement: "Ajouter une mesure",
    exportCSV: "Exporter CSV",
    addFirstMeasurement: "Ajouter la première mesure",
    noMeasurements: "Aucune mesure enregistrée",
    associatedCases: "Dossiers associés",
    noCases: "Aucun dossier trouvé",
    table: {
      type: "Type",
      value: "Valeur",
      unit: "Unité",
      site: "Site",
      date: "Date",
    },
    editDialog: {
      title: "Modifier le patient",
      desc: "Mettre à jour les informations pseudonymisées du patient",
      pseudonym: "Pseudonyme",
      ageRange: "Tranche d'âge",
      sex: "Sexe",
      male: "Masculin",
      female: "Féminin",
      selectPlaceholder: "Sélectionner",
    },
    eventDialog: {
      title: "Ajouter un événement",
      desc: "Enregistrer un événement clinique pour ce patient",
      titleLabel: "Titre",
      titlePlaceholder: "ex. : Écho-Doppler",
      eventType: "Type d'événement",
      selectType: "Sélectionner le type",
      procedure: "Procédure",
      imaging: "Imagerie",
      lab: "Résultat de laboratoire",
      note: "Note clinique",
      medication: "Changement de traitement",
      description: "Description (optionnel)",
      descPlaceholder: "Détails supplémentaires...",
    },
    measDialog: {
      title: "Ajouter une mesure",
      desc: "Enregistrer une mesure clinique",
      type: "Type",
      selectType: "Sélectionner le type",
      abi: "IPS",
      tbi: "ITO",
      diameter: "Diamètre (mm)",
      stenosis: "Sténose (%)",
      velocity: "Vélocité max (cm/s)",
      walkingDistance: "Distance de marche (m)",
      value: "Valeur",
      valuePlaceholder: "0,65",
      unit: "Unité",
      unitPlaceholder: "ex. : ratio, mm, %",
      site: "Site (optionnel)",
      sitePlaceholder: "ex. : AFS droite, ATA gauche",
    },
    toasts: {
      updated: "Mis à jour",
      updatedDesc: "Informations du patient enregistrées",
      added: "Ajouté",
      eventAdded: "Événement créé",
      measAdded: "Mesure enregistrée",
      eventDeleted: "Événement supprimé",
      eventDeletedDesc: "L'événement a été supprimé",
      measDeleted: "Mesure supprimée",
      measDeletedDesc: "La mesure a été supprimée",
      bulkDeletedDesc: "Les éléments sélectionnés ont été supprimés",
      softDeleted: "Patient déplacé dans la corbeille",
      softDeletedDesc: "a été déplacé dans la corbeille. Récupération possible pendant 30 jours.",
      undo: "Annuler",
      restored: "Patient restauré",
      restoredDesc: "Le patient a été récupéré de la corbeille",
      permanentlyDeleted: "Supprimé définitivement",
      permanentlyDeletedDesc: "Le patient et toutes les données associées ont été définitivement supprimés",
    },
    deleteDialog: {
      title: "Supprimer le patient",
      desc: "Ce patient sera déplacé dans la corbeille et pourra être récupéré pendant 30 jours. Après ce délai, toutes les données associées seront définitivement supprimées.",
      confirm: "Supprimer le patient",
      deleting: "Suppression...",
    },
    deleteEventDialog: {
      title: "Supprimer l'événement",
      desc: "Cela supprimera définitivement cet événement. Cette action est irréversible.",
      confirm: "Supprimer l'événement",
    },
    deleteMeasDialog: {
      title: "Supprimer la mesure",
      desc: "Cela supprimera définitivement cette mesure. Cette action est irréversible.",
      confirm: "Supprimer la mesure",
    },
    toastsDeleted: "Patient supprimé",
    toastsDeletedDesc: "Patient et toutes les données associées supprimés",
    trash: {
      title: "Corbeille",
      showTrash: "Corbeille",
      empty: "Aucun patient supprimé",
      emptyDesc: "Les patients supprimés apparaîtront ici pendant 30 jours avant la suppression définitive.",
      restore: "Restaurer",
      deletePermanently: "Supprimer définitivement",
      deletedOn: "Supprimé",
      expiresOn: "Expire",
      daysLeft: "j restants",
      permanentDeleteDialog: {
        title: "Supprimer définitivement le patient",
        desc: "Cela supprimera définitivement ce patient et toutes les données associées. Cette action est irréversible.",
        confirm: "Supprimer définitivement",
      },
    },
    timeAgo: {
      today: "Aujourd'hui",
      yesterday: "Hier",
      daysAgo: "j",
    },
  },

  contentGate: {
    previewBanner: "Vous naviguez en mode invité — connectez-vous pour accéder à toutes les fonctionnalités",
    title: "Envie d'utiliser ce module ?",
    subtitle: "Créez un compte gratuit en 30 secondes pour accéder à toutes les fonctionnalités, sauvegarder vos données et suivre votre progression.",
    createAccount: "Créer un compte gratuit — sans carte bancaire",
    signIn: "J'ai déjà un compte",
    benefits: [
      "Rapports cliniques assistés par IA",
      "Gestion de patients avec AquaMR Digital Twin",
      "AquaMR Registry & tableaux de performance",
      "Formation, simulation & réseau d'experts",
    ],
  },

  notifications: {
    title: "Notifications",
    empty: "Aucune notification",
    markAllRead: "Tout marquer lu",
    types: {
      forum_reply: "Nouvelle réponse à votre post forum",
      expert_response: "Un expert a répondu à votre demande",
      follow_up_reminder: "Rappel de suivi patient",
      case_update: "Mise à jour de dossier",
    },
  },
  onboarding: {
    roleTitle: "Votre rôle",
    roleDesc: "Sélectionnez le rôle qui décrit le mieux votre poste.",
    institutionTitle: "Votre établissement",
    institutionDesc: "Dites-nous où vous exercez.",
    specialtyTitle: "Votre spécialité",
    specialtyDesc: "Choisissez votre domaine d'expertise.",
    patientTitle: "Premier patient",
    patientDesc: "Créez optionnellement votre premier dossier patient.",
    institutionName: "Nom de l'établissement",
    institutionPlaceholder: "ex. Hôpital Universitaire de Genève",
    displayName: "Nom affiché",
    displayNamePlaceholder: "Dr. Marie Dupont",
    pseudonym: "Pseudonyme du patient",
    pseudonymPlaceholder: "ex. PAT-001",
    ageRange: "Tranche d'âge",
    sex: "Sexe",
    male: "Homme",
    female: "Femme",
    category: "Catégorie",
    skip: "Passer",
    optional: "optionnel",
    finish: "Terminer",
    complete: "Bienvenue ! Votre profil est configuré.",
    error: "Impossible de sauvegarder votre profil. Réessayez.",
    roles: {
      physician: "Médecin",
      trainee: "Interne",
      expert_reviewer: "Expert réviseur",
      hospital_admin: "Administrateur hospitalier",
      research_lead: "Responsable de recherche",
    },
    specialties: {
      vascular_surgery: "Chirurgie vasculaire",
      interventional_radiology: "Radiologie interventionnelle",
      cardiology: "Cardiologie",
      angiology: "Angiologie",
      general_surgery: "Chirurgie générale",
      other: "Autre",
    },
  },
  analytics: {
    title: "Analytique",
    subtitle: "Tendances, distributions et insights sur vos données cliniques",
    kpi: { patients: "Patients", cases: "Dossiers", measurements: "Mesures", aiReports: "Rapports IA" },
    measurementTrends: "Tendances des mesures",
    measurementTrendsDesc: "Valeurs moyennes dans le temps par type de mesure",
    caseDistribution: "Distribution des dossiers",
    caseDistributionDesc: "Répartition par catégorie vasculaire",
    casesOverTime: "Dossiers dans le temps",
    casesOverTimeDesc: "Croissance cumulative des dossiers",
    statusBreakdown: "Répartition par statut",
    recentActivity: "Activité récente",
    recentActivityDesc: "Derniers événements cliniques et actions",
    noMeasurements: "Aucune mesure enregistrée",
    noCases: "Aucun dossier trouvé",
    noActivity: "Aucune activité récente",
    cumulative: "Cumulatif",
    newCases: "Nouveaux dossiers",
    cases: "Dossiers",
    period: "Période",
    period7d: "7 jours",
    period30d: "30 jours",
    period90d: "90 jours",
    period1y: "1 an",
    periodAll: "Tout",
    category: "Catégorie",
    allCategories: "Toutes les catégories",
    allInstitutions: "Tous les établissements",
    noInstitution: "Sans établissement",
    institutionComparison: "Comparaison inter-institutions",
    institutionComparisonDesc: "Métriques côte à côte entre vos institutions",
    noInstitutions: "Aucune institution trouvée. Rejoignez une institution pour voir les comparaisons.",
    instMetrics: {
      cases: "Dossiers",
      outcomes: "Résultats",
      measurements: "Mesures",
    },
  },

  legal: {
    tabs: { terms: "CGU", privacy: "Confidentialité", notice: "Mentions légales" },
    contact: "Pour toute question, contactez-nous : contact@emotionscare.com",
    terms: {
      title: "Conditions Générales d'Utilisation",
      lastUpdated: "Dernière mise à jour : mars 2026",
      sections: [
        { heading: "1. Acceptation des conditions", body: "En accédant à AquaMR Flow (« la Plateforme »), vous acceptez d'être lié par les présentes CGU. Si vous n'acceptez pas, n'utilisez pas la Plateforme." },
        { heading: "2. Description du service", body: "AquaMR Flow est une plateforme d'aide à la décision clinique pour les professionnels de la médecine vasculaire. La Plateforme N'EST PAS un dispositif médical et ne fournit pas de diagnostic." },
        { heading: "3. Comptes utilisateurs", body: "Vous devez fournir des informations exactes lors de la création de votre compte. Vous êtes responsable de la confidentialité de vos identifiants." },
        { heading: "4. Utilisation acceptable", body: "Vous acceptez d'utiliser la Plateforme uniquement à des fins licites et conformément aux réglementations médicales applicables." },
        { heading: "5. Contenu généré par IA", body: "Toutes les sorties IA sont fournies à titre d'aide à la décision uniquement. Elles nécessitent la revue et la validation d'un professionnel de santé qualifié." },
        { heading: "6. Données et confidentialité", body: "Les données patients sont pseudonymisées. Nous traitons les données conformément au RGPD. Voir notre Politique de Confidentialité pour les détails." },
        { heading: "7. Propriété intellectuelle", body: "Tous les contenus et logiciels de la Plateforme sont la propriété de AquaMR Flow ou de ses concédants." },
        { heading: "8. Limitation de responsabilité", body: "La Plateforme est fournie « en l'état ». AquaMR Flow ne saurait être tenu responsable des dommages indirects liés à votre utilisation." },
        { heading: "9. Résiliation", body: "Nous pouvons suspendre ou résilier votre accès à tout moment en cas de violation des présentes CGU." },
        { heading: "10. Droit applicable", body: "Les présentes CGU sont régies par le droit de l'Union Européenne." },
      ],
    },
    privacy: {
      title: "Politique de Confidentialité",
      lastUpdated: "Dernière mise à jour : mars 2026",
      sections: [
        { heading: "1. Responsable du traitement", body: "AquaMR Flow est responsable du traitement des données collectées. Contact : privacy@aquamrflow.com" },
        { heading: "2. Données collectées", body: "Données de compte : e-mail, nom, institution, spécialité.\nDonnées cliniques : dossiers patients pseudonymisés, mesures, résultats.\nDonnées d'utilisation : interactions, journaux d'audit.\nNous NE collectons PAS de données directement identifiantes." },
        { heading: "3. Base juridique (RGPD Art. 6)", body: "Consentement : pour la création de compte.\nIntérêt légitime : pour l'amélioration et la sécurité.\nObligation légale : pour les journaux d'audit." },
        { heading: "4. Utilisation des données", body: "Fournir et améliorer les services.\nGénérer des insights anonymisés.\nMaintenir les pistes d'audit.\nCommuniquer les mises à jour." },
        { heading: "5. Partage des données", body: "Nous ne vendons pas de données personnelles. Les données peuvent être partagées avec des fournisseurs d'infrastructure, conformément à la loi, ou sous forme anonymisée pour la recherche." },
        { heading: "6. Conservation", body: "Données de compte : conservées pendant la durée d'activité du compte.\nDonnées cliniques : selon la réglementation applicable.\nJournaux d'audit : minimum 5 ans." },
        { heading: "7. Vos droits (RGPD)", body: "Vous avez le droit d'accéder, rectifier, effacer, limiter le traitement, à la portabilité et d'opposition. Contact : privacy@aquamrflow.com" },
        { heading: "8. Sécurité", body: "Chiffrement en transit (TLS) et au repos. Sécurité au niveau des lignes (RLS). Tous les accès sont journalisés." },
        { heading: "9. Cookies", body: "Nous utilisons uniquement des cookies strictement nécessaires. Aucun cookie publicitaire tiers." },
        { heading: "10. Mises à jour", body: "Nous pouvons mettre à jour cette politique. Les changements significatifs seront communiqués par e-mail ou notification." },
      ],
    },
    notice: {
      title: "Mentions Légales",
      lastUpdated: "Dernière mise à jour : mars 2026",
      sections: [
        { heading: "Éditeur", body: "EMOTIONSCARE SASU\nSIREN : 944 505 445\n80000 Amiens, France\nContact : contact@emotionscare.com" },
        { heading: "Hébergement", body: "La Plateforme est hébergée sur une infrastructure cloud dans l'Union Européenne." },
        { heading: "Avertissement médical", body: "AquaMR Flow N'EST PAS un dispositif médical. C'est un outil d'aide à la décision clinique destiné aux professionnels de santé qualifiés." },
        { heading: "Propriété intellectuelle", body: "Tous droits réservés. Le nom, le logo et le design de AquaMR Flow sont protégés." },
        { heading: "Droit applicable", body: "Les présentes mentions sont régies par le droit de l'Union Européenne." },
      ],
    },
  },

  support: {
    title: "Aide et support",
    subtitle: "Obtenez de l'aide sur votre compte et les fonctionnalités de AquaMR Flow.",
    email: { title: "Support par e-mail", desc: "Obtenez une réponse de notre équipe sous 24 heures." },
    response: { title: "Délai de réponse", desc: "Nous répondons généralement sous 24 heures ouvrées." },
    faq: {
      title: "Questions fréquentes",
      items: [
        { q: "AquaMR Flow est-il un dispositif médical ?", a: "Non. C'est un outil d'aide à la décision clinique. Toutes les sorties IA nécessitent la revue d'un clinicien qualifié." },
        { q: "Comment les données patients sont-elles protégées ?", a: "Toutes les données sont pseudonymisées. Nous utilisons la sécurité au niveau des lignes, le chiffrement et des journaux d'audit complets." },
        { q: "Puis-je exporter mes données ?", a: "Oui. Vous pouvez exporter les données patients, mesures et rapports en CSV et PDF." },
        { q: "Que se passe-t-il pendant la bêta gratuite ?", a: "Pendant la période bêta, toutes les fonctionnalités sont disponibles gratuitement. Nous vous informerons avant toute facturation." },
        { q: "Comment supprimer mon compte ?", a: "Contactez contact@emotionscare.com. Vos données seront supprimées sous 30 jours." },
      ],
    },
    links: { terms: "Conditions générales", privacy: "Politique de confidentialité" },
    contact: {
      title: "Envoyez-nous un message",
      desc: "Une question, une suggestion ou besoin d'aide ? Remplissez le formulaire et nous vous répondrons.",
      name: "Votre nom",
      namePlaceholder: "Dr. Marie Dupont",
      email: "Votre email",
      emailPlaceholder: "vous@hopital.fr",
      message: "Message",
      messagePlaceholder: "Décrivez votre question ou problème...",
      send: "Envoyer le message",
      sent: "Message envoyé ! Nous vous répondrons sous 24h.",
      error: "Impossible d'envoyer le message. Veuillez réessayer ou nous écrire directement par email.",
    },
  },

  timeAgo: {
    minutesAgo: "il y a {{count}}min",
    hoursAgo: "il y a {{count}}h",
    daysAgo: "il y a {{count}}j",
  },
  cookies: {
    message: "Nous utilisons uniquement des cookies strictement nécessaires pour l'authentification et la session. Aucun cookie publicitaire.",
    learnMore: "En savoir plus sur notre politique de confidentialité",
    accept: "Accepter",
    decline: "Refuser",
  },
  checkout: {
    success: {
      title: "Abonnement confirme !",
      desc: "Votre plan Professional est actif. Profitez d'un acces illimite a toutes les fonctionnalites.",
      goToDashboard: "Aller au tableau de bord",
      viewSettings: "Voir les parametres d'abonnement",
      verifying: "Verification de votre paiement...",
      verifyFailed: "Probleme de verification",
      verifyFailedDesc: "Nous n'avons pas pu verifier votre session de paiement. Si vous avez ete debite, votre abonnement sera active sous peu. Contactez le support si le probleme persiste.",
    },
    cancel: {
      title: "Paiement annulé",
      desc: "Aucun frais n'a été appliqué. Vous pouvez souscrire à tout moment depuis la page tarifs.",
      backToPricing: "Retour aux tarifs",
      goToDashboard: "Aller au tableau de bord",
    },
  },
  premiumGate: {
    title: "Fonctionnalité Professional",
    desc: "Cette fonctionnalité nécessite un abonnement Professional.",
    featureDesc: "{{feature}} nécessite un abonnement Professional.",
    betaNote: "Pendant la bêta, toutes les fonctionnalités sont accessibles. Souscrivez pour garder l'accès après la bêta.",
    upgrade: "Passer au Professional",
    limitReached: "Vous avez atteint la limite gratuite pour {{feature}}. Passez au Professional pour un accès illimité.",
    limitNear: "Vous avez utilisé {{current}} sur {{limit}} {{feature}} gratuits. Passez au Professional pour un accès illimité.",
    features: {
      aiReports: "rapports IA",
      patients: "patients",
    },
  },
  innovationLab: {
    title: "Labo d'innovation",
    subtitle: "Fonctionnalités à venir et capacités expérimentales en développement.",
    feedback: "Une idée ou un retour ? Contactez-nous à contact@emotionscare.com",
    status: { planned: "Planifié", concept: "Concept" },
    features: {
      federated: { title: "Apprentissage fédéré", desc: "Entraînement IA collaboratif respectueux de la vie privée — mises à jour du modèle partagées, données restent locales." },
      aiSafety: { title: "Couche de sécurité IA", desc: "MLOps de niveau médical avec versionnement des modèles, détection de dérive et suivi des corrections humaines." },
      imaging: { title: "Pipeline d'imagerie", desc: "Résumés d'imagerie, mesures structurées et outils d'annotation. Architecture compatible DICOM." },
      wearables: { title: "Objets connectés & Suivi", desc: "Application patient pour tests de marche, journal des symptômes, photos de plaies et observance de la compression." },
      arTraining: { title: "Formation immersive RA", desc: "Mode check-list RA et formation échographique par stations." },
    },
  },

  seo: {
    landing: {
      title: "AquaMR Flow — Plateforme clinique IA pour la médecine vasculaire",
      description: "Unifiez comptes rendus IA, gestion de cas patients, registre des résultats, certification, simulation et réseau d'experts pour chirurgiens vasculaires et angiologues. Gratuit pendant la bêta.",
    },
    pricing: {
      title: "Tarifs — Plans AquaMR Flow",
      description: "Comparez les plans AquaMR Flow : Individuel gratuit, Professionnel à 99 €/mois et tarif Institution sur mesure. Toutes les fonctionnalités gratuites pendant la bêta.",
    },
    support: {
      title: "Support & FAQ — AquaMR Flow",
      description: "Obtenez de l'aide avec AquaMR Flow. Contactez le support, consultez la FAQ sur les rapports IA, la gestion des patients, la certification et plus.",
    },
    auth: {
      title: "Connexion — AquaMR Flow",
      description: "Connectez-vous ou créez un compte AquaMR Flow pour accéder à tous les modules cliniques.",
    },
    legal: {
      terms: { title: "Conditions d'utilisation — AquaMR Flow", description: "Conditions d'utilisation de AquaMR Flow — règles d'usage de la plateforme clinique." },
      privacy: { title: "Politique de confidentialité — AquaMR Flow", description: "Politique de confidentialité AquaMR Flow — traitement de vos données conforme au RGPD." },
      notice: { title: "Mentions légales — AquaMR Flow", description: "Mentions légales AquaMR Flow — informations sur l'éditeur et avertissements." },
    },
    notFound: {
      title: "Page introuvable — AquaMR Flow",
      description: "La page que vous recherchez n'existe pas ou a été déplacée.",
    },
    settings: {
      title: "Paramètres — AquaMR Flow",
      description: "Gérez votre compte AquaMR Flow, profil, langue et préférences d'apparence.",
    },
    dashboard: {
      title: "Tableau de bord — AquaMR Flow",
      description: "Vue d'ensemble de vos cas, rapports IA, résultats et activité sur AquaMR Flow.",
    },
    digitalTwin: {
      title: "AquaMR Digital Twin — AquaMR Flow",
      description: "Carte vasculaire interactive et chronologie patient pour l'aide à la décision clinique.",
    },
    education: {
      title: "Formation — AquaMR Flow",
      description: "Cours de médecine vasculaire, quiz et badges numériques pour la certification continue.",
    },
    network: {
      title: "Réseau d'experts — AquaMR Flow",
      description: "Discutez de cas, consultez des experts et connectez-vous à la communauté vasculaire.",
    },
    simulation: {
      title: "Laboratoire de simulation — AquaMR Flow",
      description: "Cas cliniques simulés pour affiner vos compétences décisionnelles.",
    },
    registry: {
      title: "AquaMR Registry — AquaMR Flow",
      description: "Suivez les résultats patients, taux de complications et benchmarks qualité.",
    },
    compliance: {
      title: "Conformité — AquaMR Flow",
      description: "Piste d'audit, gestion du consentement et surveillance de la sécurité IA.",
    },
    team: {
      title: "Équipe — AquaMR Flow",
      description: "Gérez les membres et rôles de votre institution sur AquaMR Flow.",
    },
    research: {
      title: "Recherche — AquaMR Flow",
      description: "Créez des études, gérez les membres et exportez des données pour la recherche vasculaire.",
    },
    aiAssistant: {
      title: "Assistant IA — AquaMR Flow",
      description: "Générez des rapports cliniques assistés par IA avec analyse vasculaire basée sur les preuves.",
    },
    patients: {
      title: "Patients — AquaMR Flow",
      description: "Gérez les cas patients, mesures et documentation clinique.",
    },
    analytics: {
      title: "Analytique — AquaMR Flow",
      description: "Tableau de bord analytique avec tendances de cas, mesures et comparaisons institutionnelles.",
    },
    outcomes: {
      title: "Résultats patients — AquaMR Flow",
      description: "Suivi des résultats rapportés par les patients avec des questionnaires vasculaires validés.",
    },
    performance: {
      title: "Performance clinique — AquaMR Flow",
      description: "Indicateurs cliniques clés comparés aux benchmarks européens.",
    },
    riskCalculator: {
      title: "Calculateur de risque — AquaMR Flow",
      description: "Calculateurs de risque clinique : SCORE2, Wells TVP, interprétation IPS.",
    },
  },

  patientOutcomes: {
    title: "Résultats patients (PROMs)",
    subtitle: "Questionnaires validés pour l'évaluation de la qualité de vie vasculaire",
    newQuestionnaire: "Nouveau questionnaire",
    totalSubmissions: "Total soumissions",
    avgScore: "Score moyen",
    thisWeek: "Cette semaine",
    trend: "Tendance",
    improving: "↑ En amélioration",
    declining: "↓ En déclin",
    stable: "→ Stable",
    scoreTrend: "Tendance des scores",
    vascuqol6Desc: "Questionnaire de qualité de vie vasculaire — 6 items (spécifique AOMI)",
    civiq14Desc: "Questionnaire d'insuffisance veineuse chronique — 14 items",
    noSubmissions: "Aucune soumission pour l'instant",
    startFirst: "Commencer la première évaluation",
    clinicalThreshold: "Seuil clinique",
    recentSubmissions: "Soumissions récentes",
    date: "Date",
    questionnaire: "Questionnaire",
    score: "Score",
    status: "Statut",
    completed: "Complété",
    selectCase: "Sélectionner un cas patient",
    chooseCase: "Choisir un cas...",
    questionnaireType: "Type de questionnaire",
    step: "Étape",
    of: "sur",
    back: "Retour",
    next: "Suivant",
    submitting: "Envoi...",
    submitQuestionnaire: "Soumettre le questionnaire",
    submitSuccess: "Questionnaire soumis avec succès",
    submitError: "Échec de la soumission du questionnaire",
    scoreOptions: {
      none: "Aucun / Pas du tout",
      slightly: "Légèrement",
      moderately: "Modérément",
      quiteALot: "Assez fortement",
      extremely: "Extrêmement",
    },
  },

  clinicalPerformance: {
    title: "Tableau de bord de performance clinique",
    subtitle: "Indicateurs de performance clés comparés aux benchmarks vasculaires européens (ESVS / Vascunet)",
    complicationRate: "Taux de complications à 30 jours",
    meanTimeToProcedure: "Délai moyen avant intervention",
    restenosisRate: "Taux de resténose (6 mois)",
    patientSatisfaction: "Satisfaction patient",
    vsBenchmark: "vs benchmark UE",
    euBenchmark: "Benchmark UE",
    methodology: "Méthodologie & sources de données",
    complicationDesc: "Résultats marqués « complication » dans vos cas, divisés par le nombre total de cas.",
    meanTimeDesc: "Nombre moyen de jours entre la création du cas et le premier résultat d'intervention.",
    restenosisDesc: "Résultats marqués « resténose » en pourcentage du total des cas.",
    satisfactionDesc: "Score PROM normalisé moyen sur tous les questionnaires complétés.",
    benchmarkNote: "Basé sur les recommandations ESVS et les données agrégées du registre Vascunet (2023–2024).",
  },

  riskCalculator: {
    title: "Calculateur de risque vasculaire",
    subtitle: "Outils de scoring clinique interactifs avec résultats instantanés et recommandations de traitement",
    clinicalSupport: "Aide à la décision clinique",
    notSubstitute: "Ne remplace pas le jugement clinique",
    score2: {
      title: "SCORE2 — Risque CVD à 10 ans",
      desc: "Estimation du risque cardiovasculaire fatal et non fatal selon la Société Européenne de Cardiologie",
      age: "Âge (années)",
      sex: "Sexe",
      male: "Homme",
      female: "Femme",
      systolic: "Pression artérielle systolique (mmHg)",
      cholesterol: "Cholestérol total (mmol/L)",
      smoker: "Fumeur actuel",
      calculate: "Calculer le risque SCORE2",
      tenYearRisk: "Risque CVD à 10 ans",
      risk: "Risque",
      recommendation: "Recommandation thérapeutique",
      disclaimer: "Basé sur les recommandations ESC/EAS 2021. Ceci est une estimation simplifiée — utilisez les tableaux SCORE2 validés pour les décisions cliniques.",
      low: "Faible",
      moderate: "Modéré",
      high: "Élevé",
      veryHigh: "Très élevé",
      lowRec: "Modification du mode de vie. Réévaluation dans 5 ans.",
      moderateRec: "Envisager un traitement hypolipémiant. Cible LDL <2,6 mmol/L. Suivi annuel.",
      highRec: "Initier un traitement par statine. Cible LDL <1,8 mmol/L. Envisager un antihypertenseur si PAS >140.",
      veryHighRec: "Traitement hypolipémiant intensif (statine haute intensité ± ézétimibe). Cible LDL <1,4 mmol/L. Traitement antihypertenseur. Envisager un antiplaquettaire.",
    },
    wells: {
      title: "Score de Wells — Probabilité de TVP",
      desc: "Règle de prédiction clinique pour la thrombose veineuse profonde",
      score: "Score de Wells TVP",
      lowProb: "Probabilité faible",
      moderateProb: "Probabilité modérée",
      highProb: "Probabilité élevée",
      approach: "Approche recommandée",
      disclaimer: "Basé sur Wells et al. Règle de décision clinique. En pratique, combiner avec D-dimères et imagerie si approprié.",
      criteria: {
        cancer: "Cancer actif (traitement dans les 6 mois)",
        paralysis: "Paralysie, parésie ou plâtre récent du membre inférieur",
        bedridden: "Alité >3 jours ou chirurgie majeure dans les 12 semaines",
        tenderness: "Sensibilité localisée le long du système veineux profond",
        swelling: "Jambe entière enflée",
        calf: "Gonflement du mollet >3cm comparé à la jambe asymptomatique",
        pitting: "Œdème prenant le godet limité à la jambe symptomatique",
        collateral: "Veines superficielles collatérales (non variqueuses)",
        previous: "TVP précédemment documentée",
        alternative: "Diagnostic alternatif au moins aussi probable qu'une TVP",
      },
    },
    abi: {
      title: "IPS — Index de pression systolique",
      desc: "Entrez les mesures de pression artérielle systolique (mmHg)",
      rightArm: "Bras droit (mmHg)",
      leftArm: "Bras gauche (mmHg)",
      rightAnkle: "Cheville droite (mmHg)",
      leftAnkle: "Cheville gauche (mmHg)",
      calculate: "Calculer l'IPS",
      rightABI: "IPS droit",
      leftABI: "IPS gauche",
      recommendation: "Recommandation clinique",
      normal: "Normal",
      borderline: "Limite",
      pad: "Artériopathie oblitérante des membres inférieurs (AOMI)",
      severe: "AOMI sévère / Ischémie critique",
      nonCompressible: "Vaisseaux non compressibles (calcification)",
    },
  },

  fhir: {
    ready: "FHIR R4 Prêt",
    partial: "FHIR Partiel",
    na: "FHIR N/D",
    readyTooltip: "Les ressources Patient, Condition et Observation sont mappables au format FHIR R4 pour l'interopérabilité.",
    partialTooltip: "Ressources Patient et Condition disponibles. Ajoutez des mesures pour le mapping Observation complet.",
    naTooltip: "Aucune donnée clinique encore. Ajoutez des cas et mesures pour l'interopérabilité FHIR.",
  },

  researchExport: {
    button: "Export recherche",
    title: "Export de données anonymisées",
    description: "Générez des statistiques agrégées de vos cas pour soumission au registre. Aucune donnée individuelle n'est incluse.",
    gdprCompliant: "Conforme RGPD",
    aggregateOnly: "Agrégé uniquement",
    registryFormat: "Format registre",
    generate: "Générer le résumé",
    generating: "Génération...",
    download: "Télécharger .txt",
    downloadSuccess: "Rapport téléchargé",
    error: "Échec de la génération de l'export",
  },
};
