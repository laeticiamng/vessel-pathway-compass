...
export default function WebVitalsAdmin() {
  ...
  // Lazy-load WCAG audit JSON (generated by scripts/wcag_audit.py).
  const [wcag, setWcag] = useState<{ light: WCAGStats; dark: WCAGStats } | null>(null);
  useEffect(() => {
    fetch("/wcag_audit.json")
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (d?.light?.summary && d?.dark?.summary) {
          setWcag({ light: d.light.summary, dark: d.dark.summary });
        }
      })
      .catch(() => {});
  }, []);
