  ...
  const generateExport = async () => {
    ...
    try {
      const [casesRes, outcomesRes, measRes, patientsRes, promsRes] = await Promise.all([
        supabase.from("cases").select("id, category, status, created_at"),
        supabase.from("outcomes").select("id, outcome_type, outcome_date"),
        supabase.from("measurements").select("id, measurement_type, value, unit"),
        supabase.from("patients").select("id, age_range, sex, risk_factors").is("deleted_at", null),
  ...
  return (
    <>
      <Button variant="outline" onClick={() => { setOpen(true); setReport(null); }}>
        <FileText className="h-4 w-4 mr-2" /> Research Export
      </Button>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-2xl max-h-[80vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription>
              Generate aggregate statistics from your cases for registry submission. No individual patient data is included.
            </DialogDescription>
          </DialogHeader>
          ...
          {!report ? (
            <div className="flex flex-col items-center gap-4 py-8">
              <div className="flex gap-2">
                <Badge variant="outline"><Shield className="h-3 w-3 mr-1" /> GDPR Compliant</Badge>
                <Badge variant="outline">Aggregate Only</Badge>
                <Badge variant="secondary">Registry Format</Badge>
              </div>
